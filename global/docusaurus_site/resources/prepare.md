...
- MUST run `npx create-docusaurus@latest . classic` inside the cloned repository root (do NOT create a subfolder for the site)
...
4. Inside `siteDir`:
   ...
   - Otherwise, execute: `npx create-docusaurus@latest . classic` (create the site in the repository root; do NOT create a subfolder).
...
## Verification
- Confirm that `<workspace>/<repo>/package.json` exists.
- Confirm presence of Docusaurus config file in `<workspace>/<repo>/`.
